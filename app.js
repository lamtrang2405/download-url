(function () {
  ...
  function downloadAll(urls, timeoutSec, zip) {
    ...
    function next() {
      ...
      return fetch(url, {
        ...
        var base = sanitizeFilename(url, i);
  ...
  function run() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (fileInput && fileInput.files && fileInput.files.length) {
        ...
        file.arrayBuffer().then(function (buf) {
          urls = extractUrlsFromExcel(
            buf,
            (document.getElementById('url_column') || {}).value ? document.getElementById('url_column').value.trim() : null,
            sheetName
          );
          go(urls, done);
        }).catch(function (err) {
      ...
      if (!urls.length) {
        showMessage('No URLs found. Paste one URL per line (e.g. https://example.com/file.jpg).', 'error');
        done();
        return;
      }
    ...
    function go(urls, done) {
      showMessage('Downloading ' + urls.length + ' link(s)...', 'success');
      ...
      var zip = new JSZip();
      var resultsRef;
      downloadAll(urls, timeout, zip).then(function (results) {
        resultsRef = results;
        ...
        showMessage('Done: ' + ok + ' of ' + urls.length + ' downloaded. ZIP saved.', 'success');
      }).catch(function (err) {
